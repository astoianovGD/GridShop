...
class BucketServiceTest {
    ...
    void addProductToBucket_WhenBucketExists_ShouldExecuteInsert() {
        ...
        when(customJdbcTemplate.findOne(anyString(), any(Function.class), eq(userId))).thenReturn(bucketId);

        bucketService.addProductToBucket(userId, productId, amount);

        verify(customJdbcTemplate, times(1)).execute(anyString(), eq(bucketId), eq(productId), eq(amount));
    }
    ...
    void addProductToBucket_WhenBucketNotExists_ShouldCreateBucketAndExecuteInsert() {
        ...
        when(customJdbcTemplate.findOne(anyString(), any(Function.class), eq(userId)))
                .thenReturn(null)
                .thenReturn(bucketId);

        bucketService.addProductToBucket(userId, productId, amount);

        verify(customJdbcTemplate, times(1)).execute(eq("INSERT INTO bucket (user_id) VALUES (?) ON CONFLICT (user_id) DO NOTHING"), eq(userId));
        verify(customJdbcTemplate, times(1)).execute(anyString(), eq(bucketId), eq(productId), eq(amount));
    }
    ...
    void removeProductFromBucket_WhenBucketExists_ShouldExecuteDelete() {
        ...
        verify(customJdbcTemplate, times(1)).execute(anyString(), eq(bucketId), eq(productId));
    }
    ...
    void removeProductFromBucket_WhenBucketNotExists_ShouldDoNothing() {
        ...
        when(customJdbcTemplate.findOne(anyString(), any(Function.class), eq(userId))).thenReturn(null);
    ...
    void getProductsFromBucket_WhenBucketExists_ShouldReturnProducts() throws SQLException {
        ...
        when(customJdbcTemplate.findMany(anyString(), any(Function.class), eq(bucketId))).thenAnswer(invocation -> {
    ...
    void clearBucket_WhenBucketExists_ShouldExecuteDelete() {
        ...
        verify(customJdbcTemplate, times(1)).execute(anyString(), eq(bucketId));
    }
