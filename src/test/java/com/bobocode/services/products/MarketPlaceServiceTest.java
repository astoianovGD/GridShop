...
class MarketPlaceServiceTest {
    ...
    void getProductById_WhenExists_ShouldReturnProduct() {
        when(customJdbcTemplate.findOne(anyString(), any(), eq(1L))).thenReturn(testProduct);
    ...
    void getProductById_WhenNotExists_ShouldThrowException() {
        when(customJdbcTemplate.findOne(anyString(), any(), eq(99L))).thenReturn(null);
    ...
    void removeProduct_ShouldDeactivateAndRemoveFromBucket() {
        ...
        verify(customJdbcTemplate, times(2)).execute(sqlCaptor.capture(), eq(1L));
