...
class OrderServiceTest {
    ...
    void createOrderFromBucket_WhenBucketExists_ShouldCreateOrderAndClearBucket() {
        ...
        when(customJdbcTemplate.findOne(contains("FROM bucket"), any(Function.class), eq(userId))).thenReturn(bucketId);
        when(customJdbcTemplate.findOne(contains("INSERT INTO orders"), any(Function.class), eq(userId))).thenReturn(orderId);
    ...
    void getOrderHistory_ShouldReturnFormattedStrings() throws SQLException {
        ...
        when(customJdbcTemplate.findMany(anyString(), any(Function.class), eq(userId))).thenAnswer(invocation -> {
