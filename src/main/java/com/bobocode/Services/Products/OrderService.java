...
public final class OrderService {
    ...
    public void createOrderFromBucket(final long userId) {
        ...
        Long bucketId = jdbcTemplate.findOne(getBucketIdSql, rs -> {
            try {
        ...
        Long orderId = jdbcTemplate.findOne(createOrderSql, rs -> {
            try {
        ...
        String moveItemsSql = "INSERT INTO order_items "
                ...
                + "WHERE bi.bucket_id = ?";
        jdbcTemplate.execute(moveItemsSql, orderId, bucketId);

        String clearBucketSql =
                "DELETE FROM bucket_items WHERE bucket_id = ?";
        jdbcTemplate.execute(clearBucketSql, bucketId);
    }
