    ...
    public final class StaffMenu {
        ...
        @NonNull private final MarketPlaceService marketPlaceService;
        @NonNull private final CatalogMenu catalogMenu;
        @NonNull private final UserConsoleViewService userConsoleViewService;
        @NonNull private final BucketService bucketService;
        @NonNull private final OrderService orderService;
        @NonNull private final CategoryService categoryService;
        @NonNull private final ProductsConsoleViewService productsConsoleViewService;
        @NonNull private final BucketConsoleViewService bucketConsoleViewService;
        @NonNull private final OrderConsoleViewService orderConsoleViewService;
        ...
        private void handleAddProduct(final Scanner scanner) {
            ...
            if (categories.isEmpty()) {
                System.out.println("Error: No categories found in the system! Please add categories first.");
                return;
            }

            System.out.println("\n--- Available Categories ---");
            categories.forEach(c -> System.out.println("ID: " + c.getId() + " | Name: " + c.getName()));

            long categoryId;
            while (true) {
                categoryId = InputValidator.getValidId(scanner, "Enter category ID for the product:");
                if (categoryService.isCategoryExists(categoryId)) {
                    break;
                }
                System.out.println("Category with ID " + categoryId + " does not exist! Try again.");
            }
        ...
        private void handleRemoveProduct(final Scanner scanner) {
            ...
            try {
                ...
                if (!affectedUsers.isEmpty()) {
                    System.out.println("This product was removed from the bucket of following users:");
                    affectedUsers.forEach(user ->
                            System.out.printf(" - ID: %d | Name: %s %s%n", user.getId(), user.getFirstname(), user.getLastname())
                    );
                } else {
                    System.out.println("No users had this product in their bucket.");
                }
        ...
        private void handleUserSubMenu(final Scanner scanner, final UserDto user) {
            while (true) {
                ...
                switch (option) {
                    case "1" -> {
                        var cart = bucketService.getProductsFromBucket(user.getId());
                        if (cart.isEmpty()) {
        ...
        private void handleManageCategories(final Scanner scanner) {
            while (true) {
                ...
                switch (option) {
                    case "1" -> {
                        String name = InputValidator.getValidName(scanner, "Category");
                        try {
        ...
        private void handleViewCategories(final Scanner scanner) {
            while (true) {
                ...
                switch (option) {
                    case "1" -> {
                        long id = InputValidator.getValidId(scanner, "Enter category ID to change:");
                        String newName = InputValidator.getValidName(scanner, "Category");

                        try {
                            CategoryDto categoryDto = new CategoryDto();
                            categoryDto.setName(newName);

                            categoryService.editCategory(categoryDto, id); // Виправлено задвоєння
                    ...
                    case "2" -> {
                        long id = InputValidator.getValidId(scanner, "Enter category ID to remove:");
                        try {
        ...
        private void editProductInfoMenu(final Scanner scanner, final long productId) {
            ...
            switch (option) {
                case "1" -> {
                    ...
                    marketPlaceService.updateProductField(productId, p -> p.setName(newName));
                    System.out.println("Name successfully updated!");
                }
                case "2" -> {
                    BigDecimal newPrice = InputValidator.getValidPrice(scanner, "Enter new price:");
                    marketPlaceService.updateProductField(productId, p -> p.setPrice(newPrice));
                ...
                case "3" -> {
                    ...
                    long newCategoryId;
                    while (true) {
                        newCategoryId = InputValidator.getValidId(scanner, "Enter new category ID:");
                        if (categoryService.isCategoryExists(newCategoryId)) {
                            break;
                        }
                        System.out.println("Category with ID " + newCategoryId + " does not exist! Try again.");
                    }

                    Category category = categoryService.getCategoryEntityById(newCategoryId);

                    marketPlaceService.updateProductField(productId, p -> p.setCategory(category));
                    System.out.println("Category successfully updated!");
                }
                default -> System.out.println("Invalid Option!");
            }
